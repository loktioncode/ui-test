...
function Filters({ data, onFilterChange }: {
  ...
  const [filters, setFilters] = useState({
    extractData: false,
    monitor: false,
    selectedSites: [], // New state for selected sites
    selectedCategories: [],
  ...
  const handleCategorySelect = (selectedCategory: string) => {
    if (!filters.selectedCategories.includes(selectedCategory)) {
      setFilters({ ...filters, selectedCategories: [...filters.selectedCategories, selectedCategory] });
    }
  };

  const removeCategory = (selectedCategory: string) => {
    setFilters({ ...filters, selectedCategories: filters.selectedCategories.filter(category => category !== selectedCategory) });
  };
